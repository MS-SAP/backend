...
import {
    sendCourseUpcomingReminderInstructor,
    sendCourseUpcomingReminderParticipant
...
export default async function execute(manager: EntityManager) {
    logger.info(
        "CourseReminder job: looking for subcourses with first lecture in two days..."
    );
...
async function sendUpcomingCourseReminders(manager: EntityManager) {
    ...
    for (let course of courses) {
        // find any subcourses
        const subcourses = await manager.find(Subcourse, {
            course: course,
            published: true
        });
        ...
        for (let subcourse of subcourses) {
            ...
            if (moment(firstLecture.start).isSame(dayAfterTomorrow, "day")) {
                ...
                logger.info(
                    "Found lecture on (sub)course " +
                        course.name +
                        " with start date in 2 days: " +
                        firstLecture.start.toDateString()
                );
                logger.info(
                    "Sending reminders to instructor and " +
                        subcourse.participants.length +
                        " participants"
                );
