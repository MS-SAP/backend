...
import {
    sendFeedbackRequestStudent,
    sendFeedbackRequestPupil
} from "../../../common/mails/feedback-request";
...
export default async function execute(manager: EntityManager) {
    logger.info(
        "FeedbackRequest job: looking for matches that were created 30 days ago"
    );
...
async function getStudentsForFeedbackRequest(
...
async function getPupilsForFeedbackRequest(
    manager: EntityManager
): Promise<Match[]> {
    const preselectedMatches = await manager
        .createQueryBuilder()
...
async function sendFeedbackRequestsToStudents(
    manager: EntityManager,
    matches: Match[]
) {
    try {
        ...
        if (e.statusCode === mailjet.ErrorCodes.RATE_LIMIT) {
            ...
            logger.info(
                "Hit rate limit while sending feedback requests to students -> the missing mails will be sent tomorrow..."
            );
            return;
        }
        throw e;
...
async function sendFeedbackRequestsToPupils(
    manager: EntityManager,
    matches: Match[]
) {
    try {
        ...
        if (e.statusCode === mailjet.ErrorCodes.RATE_LIMIT) {
            //handle rate limit errors in mailjet
            logger.info(
                "Hit rate limit while sending feedback requests to pupils -> the missing mails will be sent tomorrow..."
            );
            return;
        }
        throw e;
