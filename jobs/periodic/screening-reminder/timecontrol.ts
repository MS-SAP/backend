...
function isHolidayAtDate(holiday: Holidays.Holiday, date: Date) {
    ...
    const holidayDuration = Math.ceil(
        moment(holiday.end).clone().diff(startOfStart, "days", true)
    );
...
// ----------------------------------------------------
function isCommonHolidayInAnyStateOfGermany(
    date: Date
    ...
    const allHolidays: Holidays.Holiday[] = allStates.reduce(
        (a, s) =>
            a.concat(...new Holidays(de, s).getHolidays(date.getFullYear())),
        []
    );
...
function performCheckForCommonHolidayInGermany(atDate: Date): boolean {
    ...
    if (hd) {
        logger.info(
            `Will not send screening reminders today, because there's a public holiday in Germany, to be specific: ${hd.name}`
        );
        return false;
    }
...
function performCheckForWeekends(atDate: Date): boolean {
    ...
    if (isWeekend) {
        logger.info(
            `Will not send screening reminders today, because it's weekend...`
        );
    }
...
export function shouldRemindAtDate(date: Date): boolean {
    return performCheckForCommonHolidays(date) && performCheckForWeekends(date);
}
