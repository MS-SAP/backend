        ...
        if (
            ...
            let params = {
                endDate:
                    ...
                    (moment().format("X") as string),
                subjects:
                    (req.query.subjects as string) ||
                    "Mathematik, Biologie, Altgriechisch",
                hoursPerWeek:
                    Number.parseFloat(req.query.hoursPerWeek as string) || 0.0,
                hoursTotal:
                    Number.parseFloat(req.query.hoursTotal as string) || 0.0,
                medium:
                    (req.query.medium as string) ||
                    "Taschenrechner mit Programmierfunktion",
                categories:
                    (req.query.categories as string) || "Liste\nListe2\nListe3"
            };
            ...
            let certificate = await generateCertificate(
                res.locals.user,
                req.params.student,
                req.params.pupil,
                params,
...
export async function confirmCertificateHandler(req: Request, res: Response) {
    let status;
    if (
        req.params.certificateId != undefined ||
        "" + req.params.certificateId
    ) {
        return res.send(
            await viewParticipationCertificate(req.params.certificateId)
        );
    }
    status = 500;
    res.status(status).end();
...
async function generateCertificate(
    ...
    let match = await entityManager.findOne(Match, {
        student: requestor,
        uuid: matchuuid
    });
    if (match == undefined) {
...
function createPDFBinary(
    ...
    html = html.replace(
        /%NAMESCHUELER%/g,
        escape(pupil.firstname + " " + pupil.lastname)
    );
    html = html.replace("%DATUMHEUTE%", moment().format("D.M.YYYY"));
    html = html.replace(
        "%SCHUELERSTART%",
        moment(startDate, "X").format("D.M.YYYY")
    );
    html = html.replace(
        "%SCHUELERENDE%",
        moment(params.endDate, "X").format("D.M.YYYY")
    );
    html = html.replace(
        "%SCHUELERFAECHER%",
        escape(params.subjects).replace(/,/g, ", ")
    );
    html = html.replace(
        "%SCHUELERFREITEXT%",
        escape(params.categories).replace(/(?:\r\n|\r|\n)/g, "<br />")
    );
...
async function viewParticipationCertificate(certificateId) {
    ...
    try {
        certificate = await entityManager.findOne(
        ...
        html = html.replace(
            "%DATUMHEUTE%",
            moment(certificate.certificateDate).format("D.M.YYYY")
        );
        html = html.replace(
            "%SCHUELERSTART%",
            moment(certificate.startDate).format("D.M.YYYY")
        );
        html = html.replace(
            "%SCHUELERENDE%",
            moment(certificate.endDate).format("D.M.YYYY")
        );
        html = html.replace(
            "%SCHUELERFAECHER%",
            escape(certificate.subjects).replace(/,/g, ", ")
        );
        html = html.replace(
            "%SCHUELERFREITEXT%",
            escape(certificate.categories).replace(/(?:\r\n|\r|\n)/g, "<br />")
        );
        html = html.replace(
            "%SCHUELERPROWOCHE%",
            escape(certificate.hoursPerWeek)
        );
        ...
        html = html.replace(
            ...
            escape(
                screeningDate
                    ? moment(screeningDate).format("D.M.YYYY")
                    : "[UNBEKANNTES DATUM]"
            )
        );
    } catch (e) {
